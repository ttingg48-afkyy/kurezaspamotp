#!/usr/bin/env python3
# main_engine.py - Kureza Spam OTP Engine
...
def process_target(api, target62, ip, idx, total):
 ...
 # ==================== HANDLER 20: BEAUTYHAUL ====================
        ...
        # ==================== JSON HANDLERS ====================
            ...
            rand_name = ''.join(random.choices(string.ascii_lowercase, k=random.randint(4, 8))).capitalize()
            rand_email = f"{rand_name.lower()}{random.randint(100,999)}@gmail.com"
            rand_password = 'Pass' + ''.join(random.choices(string.ascii_letters + string.digits, k=6)) + '@1'
...
def run_infinite_loop(target=None, callback=None):
    """Jalankan Infinite Loop dengan jeda 15 detik"""
    global stop_flag, global_callback
    stop_flag = False
    global_callback = callback
    ...
    print(f"{Fore.CYAN}[INFO]{Style.RESET_ALL} Menjalankan Infinite Loop (delay {DELAY_BETWEEN_ROUNDS} detik)...")
    ...
    try:
        while not stop_flag:
            ...
            print(f"{Fore.CYAN}[INFO]{Style.RESET_ALL} Menunggu {DELAY_BETWEEN_ROUNDS} detik...")
