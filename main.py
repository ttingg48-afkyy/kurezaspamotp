...
def show_execution_table(data, tick=0):
    ...
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  {Fore.GREEN}API Total :{Style.RESET_ALL} {Fore.YELLOW}{data.get('total_api', 85)}{' ' * (58 - 12 - len(str(data.get('total_api', 85))))}{Fore.CYAN}│{Style.RESET_ALL}")
...
def update_exec_data(name, status, detail=""):
    ...
    if status == "SUCCESS":
        ...
        exec_data['failed'] += 1
    exec_data['sent'] += 1
    total = exec_data.get('total_api', 85)
